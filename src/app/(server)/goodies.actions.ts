...
function computeRelevance(g: GoodieDto): number {
  const now = Date.now();
  const dateRef = g.date ? new Date(g.date).getTime() : new Date(g.createdAt).getTime();
...
export async function listGoodies(): Promise<GoodieDto[]> {
  ...
  const voteMap = new Map<string, number>((voteSums as any[]).map((v: any) => [v.goodieId, v._sum.value || 0]));
  ...
  const withRelevance = mapped.map(m => ({ data: m, relevance: computeRelevance(m) }));
...
export async function createGoodie(input: {
  ...
  const imageBytes = input.image ? Buffer.from(input.image as ArrayBuffer) : undefined;
  ...
  const created = await (prisma as any).goodie.create({
    ...
    select: { id: true, name: true, location: true, instructions: true, type: true, date: true, registrationUrl: true, createdAt: true },
  });
  await broadcast({
    type: "goodie_created",
    goodie: {
...
export async function voteGoodie(goodieId: string, value: -1 | 1) {
  ...
  const agg = await (prisma as any).goodieVote.aggregate({ where: { goodieId }, _sum: { value: true } });
  const totalScore = agg?._sum?.value || 0;
  await broadcast({ type: "goodie_updated", goodie: { id: goodieId, totalScore } });
  return { ok: true };
}
...
export async function toggleCollected(goodieId: string, collected: boolean) {
  ...
  if (collected) {
    try {
  await (prisma as any).goodieCollection.create({ data: { userId, goodieId } });
    } catch {}
  } else {
  await (prisma as any).goodieCollection.deleteMany({ where: { userId, goodieId } });
  }
  const collectedCount = await (prisma as any).goodieCollection.count({ where: { goodieId } });
  await broadcast({ type: "goodie_collected", goodieId, collectedCount });
  return { ok: true };
}

export async function getGoodieImage(goodieId: string) {
  const g = await (prisma as any).goodie.findUnique({ where: { id: goodieId }, select: { imageBytes: true } });
...
export async function deleteGoodie(id: string) {
  const userId = await getUserId();
  if (!userId) throw new Error("Unauthorized");
  const existing = await (prisma as any).goodie.findUnique({ where: { id }, select: { createdById: true } });
...
export async function updateGoodie(id: string, patch: {
  name?: string;
  location?: string;
  instructions?: string;
  type?: GoodieType;
  ...
  if (patch.registrationUrl !== undefined) data.registrationUrl = patch.registrationUrl;
  if (patch.image !== undefined) {
    if (patch.image === null) data.imageBytes = null; else data.imageBytes = Buffer.from(patch.image as ArrayBuffer);
  }

  const updated = await (prisma as any).goodie.update({
    where: { id },
    data,
    select: { id: true, name: true, location: true, instructions: true, type: true, date: true, registrationUrl: true, updatedAt: true },
  });
  await broadcast({
    type: "goodie_edited",
    goodie: {
