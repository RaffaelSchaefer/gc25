...
// remove leading "analysis:" / "final:" incl. **Final:**, Final —, etc.
function stripLeadTags(s?: string) {
    if (!s) return "";
    return s.replace(
        /^\s*(\*{0,2})?(analysis|final)(\*{0,2})?\s*[:\-—]\s*/i,
        ""
    );
}

/** Lucide-Icon passend zum Summary-Typ (Search nutzt jetzt Action/Wrench) */
...
/** Tools als Task rendern – nur Server-Action-Output, sonst Skeleton */
function renderToolAsTask(
    part: any,
    keyBase: string,
    _formatDateTimeFn: (iso: string) => string,
...
export default function DashboardOverview({ days, goodies = [] }: Props) {
    ...
    useEffect(() => {
        try {
            const parts =
                messages.flatMap((m) =>
                    (m.parts as any[]).filter(isToolUIPart)
                ) ?? [];
            parts.forEach(async (p: any) => {
                const id = p?.toolCallId as string | undefined;
                if (!id) return;
                if (p?.state !== "output-available") return; // nur final zusammenfassen
                ...
                const slim: ToolCallPart = {
                    ...
                    toolCallId: id,
                    state: p.state,
                    input: undefined, // kein Input an die Server-Action senden
                    output: p.output,
                };
            ...
            <div className="container mx-auto px-4">
                ...
                <div className="mb-8 text-center">
                    ...
                    <p className="text-muted-foreground mt-2">
                        Schneller Überblick, praktische Links und aktuelle
                        Events.
                    </p>
                </div>
                        ...
                        <Card className="group relative overflow-hidden border-0 ring-1 ring-fuchsia-500/20 bg-gradient-to-br from-fuchsia-500/5 via-background/60 to-background/50 backdrop-blur-xl transition-all duration-300">
                            ...
                            <CardContent className="space-y-4">
                                <p className="text-sm text-muted-foreground leading-relaxed">
                                    Öffne den Assistenten, um Fragen zu Events,
                                    Goodies oder Planung zu stellen.
                                </p>
                                <Button
                                    onClick={() => {
                            ...
                            <CardContent className="space-y-4">
                                {upcoming ? (
                                    ...
                                    <div className="text-muted-foreground">
                                        Noch keine kommenden Events – lege
                                        direkt eins im Planer an.
                                    </div>
                                )}
                            </CardContent>
                        </Card>
                    ...
                    <div className="break-inside-avoid mb-6">
                        <Card className="relative overflow-hidden border-0 ring-1 ring-orange-500/20 bg-gradient-to-br from-orange-500/5 via-background/60 to-background/50">
                            ...
                            <CardContent className="space-y-2">
                                ...
                                >
                                    <span className="inline-flex items-center gap-2 text-sm">
                                        <Music2 className="w-4 h-4 text-orange-300" />{" "}
                                        Spotify Playlist
                                    </span>
                                    <span className="text-xs text-orange-300">
                                        Öffnen →
