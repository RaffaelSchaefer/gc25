...
const EventCard = dynamic(
  () => import("@/components/event-card").then((m) => m.EventCard),
  { ssr: false },
);
const GoodieCard = dynamic(
  () => import("@/components/goodie-card").then((m) => m.GoodieCard),
  { ssr: false },
);

export function AIChatCardPart({ part }: { part: any }) {
  if (
    part.type === "tool-getEventInformation" &&
  ...
  if (
    part.type === "tool-getGoodieInformation" &&
    part.state === "output-available" &&
    part.output
  ) {
    return (
